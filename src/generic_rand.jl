...
"""
function sample3D_r!(rng::AbstractRNG, d::AbstractDensity, x::AbstractArray{<:Real})
    rand!(rng, x)
    x .= quantile3D.(d, x)
    return x
end
...
"""
function sample3D_r(rng::AbstractRNG, d::AbstractDensity, dims::Integer...)
    x = rand(rng, dims...)
    x .= quantile3D.(d, x)
    return x
end
sample3D_r(d::AbstractDensity, dims::Integer...) = 
