...
function Σ(d::GeneralIsothermal{T},r::S) where {T,S<:Real}
    ...
    α>=3 ? throw(DomainError(α,"Σ is only finite for α<3.")) : sqrt(U(π)) * ρ0 * rs^α *
...
function ∇Σ(d::GeneralIsothermal{T},r::S) where {T,S<:Real}
    ...
    ρ0,rs,α = params(d)
    ρ0,rs,α,r = promote(ρ0,rs,α,r)
    α>=3 ? throw(DomainError(α,"Σ is only finite for α<3.")) : -sqrt(U(π)) * ρ0 * rs^α * (α-1) *
...
function invΣ(d::GeneralIsothermal{T},x::S) where {T,S<:Real}
    ...
    elseif x<=0
        ...
        (sqrt(U(π)) * ρ0 * rs^α * gamma((α-1)/2) / gamma(α/2) / x)^(α-1)
    end
end
function M(d::GeneralIsothermal{T},r::S) where {T,S<:Real}
    ...
    ρ0,rs,α = params(d)
    ρ0,rs,α,r = promote(ρ0,rs,α,r)
    α >= 3 ? throw(DomainError(α,"Enclosed mass is only finite for α<3.")) : 4 * U(π) * ρ0 * rs^α * r^(3-α) / (3 - α)
end
function ∇M(d::GeneralIsothermal{T},r::S) where {T,S<:Real}
    U = promote_type(T,S)
    ρ0,rs,α = params(d)
    ρ0,rs,α,r = promote(ρ0,rs,α,r)
    α >= 3 ? throw(DomainError(α,"Enclosed mass is only finite for α<3.")) : 4 * U(π) * ρ0 * rs^α * r^(2-α)
end
function invM(d::GeneralIsothermal{T},x::S) where {T,S<:Real}
    U = promote_type(T,S)
    ρ0,rs,α = params(d)
    ρ0,rs,α,x = promote(ρ0,rs,α,x)
    α >= 3 ? throw(DomainError(α,"Enclosed mass is only finite for α<3.")) : (x * (3 - α) / (4 * U(π) * ρ0 * rs^α))^(1/(3-α))
end
# Mtot
function Mproj(d::GeneralIsothermal{T},r::S) where {T,S<:Real}
    ...
    ρ0,rs,α = params(d)
    ρ0,rs,α,r = promote(ρ0,rs,α,r)
    α >= 3 ? throw(DomainError(α,"Mproj is only finite for α<3.")) : 2 * U(π^(3/2)) * r^(3-α) * rs^α * ρ0 * gamma((α-1)/2) / (3-α) / gamma(α/2)
end
function ∇Mproj(d::GeneralIsothermal{T},r::S) where {T,S<:Real}
    U = promote_type(T,S)
    ρ0,rs,α = params(d)
    ρ0,rs,α,r = promote(ρ0,rs,α,r)
    α >= 3 ? throw(DomainError(α,"Σ is only finite for α<3.")) : 2 * U(π^(3/2)) * r^(2-α) * rs^α * ρ0 * gamma((α-1)/2) / gamma(α/2)
end
...
function Φ(d::GeneralIsothermal{T},r::S) where {T,S<:Real}
    ...
    if α == 2
        4 * U(π) * U(constants.Gvelkpc) * ρ0 * rs^α * log(r/rs)# / (3 - α) 
    else
